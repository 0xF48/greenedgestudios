...
export function NavButton() {
	...
	return <>
		{createPortal(<div onClick={() => { setIsOpen(false) }} className={cn("z-10 w-full h-full fixed transition-opacity duration-200 left-0 top-0 bg-main-100/90 backdrop-blur-xl", isOpen ? ' opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none')} ></div >, document.body)}
		<MotionBox
			resetKey={resetKey}
			relativeContentPosition={false}
			useAbsoluteOffset={false}
			classNames={{
				style: cn(isOpen ? 'bg-white' : 'bg-stone-100', 'ring ring-3 ring-white transition-colors rounded-2xl cursor-pointer z-40 hover:bg-white'),
				position: position
			}}>
			{content}
		</MotionBox >
